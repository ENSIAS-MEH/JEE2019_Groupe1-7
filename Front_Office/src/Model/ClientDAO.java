...
public class ClientDAO {
    ...
    public boolean TestLogin(String login) throws SQLException
    {
      ...
      public boolean TestEmail(String email) throws SQLException
    {
        rs=st.executeQuery("select count(*) from Client where email_client='"+email+"';");
        rs.first();
        if(rs.getInt(1)==0)
            return false;
        else 
    ...
    public Vector<Client> getAllClients() throws SQLException, ClassNotFoundException
    {
     ...
     {
         Client c=new Client();
          rs=st.executeQuery("select * from client where login='"+login+"';");
          rs.next();
        c.setLogin(rs.getString("login"));
        c.setNomClient(rs.getString("nom_client"));
        c.setPrenomClient(rs.getString("prenom_client"));
